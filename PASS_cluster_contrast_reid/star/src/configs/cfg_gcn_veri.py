...
star_path='PASS-reID-main/PASS_cluster_contrast_reid/star/'
# data locations
prefix = 'PASS-reID-main/PASS_cluster_contrast_reid/star/data'
train_name = 'source'
test_name = 'target'
loss='FOCAL LOSS'
knn = 50
...
train_data = dict(feat_path=osp.join(prefix, 'features',
                  ...
                  scores_path=osp.join(prefix, 'scores',
                                    '{}.npy'.format(train_name)),
...
# model
model = dict(type='gcn',
             kwargs=dict(feature_dim=384, nhid=512, nclass=1, dropout=0.))
...
lr_config = dict(
    policy='step',
    step = [int(r * total_epochs) for r in [0.4, 0.8, 0.9]]
)
