...
from star.utils import (load_data, dump_data, mkdir_if_no_exists, Timer)
    ...
    if not os.path.isfile(knn_path) or is_rebuild:
        ...
        if knn_method == 'hnsw':
            ...
            index = knn_faiss(feats,
                              ...
                              rebuild_index=True)
        elif knn_method=='knn_new':

            index=knn_new(feats,scores,
                              k,
                              index_path,
                              omp_num_threads=num_process,
                              rebuild_index=True)
...
def cal_sims_nbrs(features,class_scores,k):
    # print(features[30])
    features = l2norm(features)
    dis1 = pairwise_distances(features,metric='cosine')
    dis2 = pairwise_distances(class_scores, metric='cosine')
    dis = 0.5 * dis1 + 0.5 * dis2
    sims = 1 - dis

    indexs = np.argsort(-sims, axis=1)
    indexs = indexs[:, :k]
    my_sims = []
    for i in range(indexs.shape[0]):
        my_sims.append(sims[i, indexs[i]])

    return indexs, np.array(my_sims)
class knn_new(knn):
    def __init__(self,
                 feats,scores,
                 k,
        ...
        knn_ofn = index_path + '.npz'
        if os.path.exists(knn_ofn):
            print('[knn_new] read knns from {}'.format(knn_ofn))
            self.knns = np.load(knn_ofn)['data']
        else:
            nbrs, sims = cal_sims_nbrs(feats,scores,k)
            self.knns = [(np.array(nbr),
