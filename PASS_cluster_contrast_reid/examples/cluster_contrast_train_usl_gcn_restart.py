...
def relabel(labels,indexes):
    choose_labels=labels[indexes]
    ncs=len(set(choose_labels))
    relabel_dict={}
    new_labels=[]
    count=0
    count_=0
    for label in labels:
        if label not in relabel_dict:

            if label in choose_labels:
                relabel_dict[label]=count
                new_labels.append(count)
                count = count + 1
            else:
                # relabel_dict[label] = ncs+count_
                new_labels.append(-1)
                # count_ = count_ + 1
        else:
            new_labels.append(relabel_dict[label])
    return new_labels,ncs
...
def main_worker(args):
    ...
    for epoch in range(args.epochs):
        ...
        if epoch not in [16,17,18,19,20]:
            with torch.no_grad():
            ...
            #if (epoch%5==0 and epoch > 9) or epoch==21:
                # if epoch>1:
                    # labels = np.load('gcn_labels.npy')
                indexes = get_index(pseudo_labels)

                clu_train_labels = pseudo_labels[indexes]
                with open('/PASS-reID-main/PASS_cluster_contrast_reid/star/data/labels/source.meta', 'w') as f:
                    for label in clu_train_labels:
                        f.write(str(label) + '\n')
                print('save clu label done')
                pseudo_labels_gcn, indexes = run_cluster(features[indexes], None,features)
                pseudo_labels, _ = relabel(pseudo_labels_gcn, indexes)
                np.save('gcn_labels.npy',pseudo_labels)
                pseudo_labels=np.load('gcn_labels.npy')
            ...
            print('Epoch {} has {} labeled samples of {} ids and {} unlabeled samples'.
              format(epoch, len(pseudo_labeled_dataset), num_cluster, len(pseudo_outlier_dataset)))
        ...
        if epoch == 15:
            ...
            if args.optimizer == 'AdamW':
                ...
                optimizer = torch.optim.SGD(params, lr=0.00055, momentum=0.9, weight_decay=args.weight_decay)
