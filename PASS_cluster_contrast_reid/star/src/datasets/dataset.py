...
from star.utils import (read_meta, read_probs, l2norm, build_knns,
...
class GCNDataset(object):
    def __init__(self, cfg):
        ...
        scores_path=cfg.get('scores_path',None)
        ...
        with Timer('read meta and feature'):
            ...
            if scores_path is not None and cfg.knn_method=='knn_new':
        ...
        with Timer('read knn graph'):
            ...
            print('build knn graph: {}'.format(
                    knn_graph_path))
            # print(cfg.name)
            # print(cfg)
            # print(cfg.prefix)
            knn_prefix = os.path.join(cfg.prefix, 'knns', cfg.name)
            self.knns = build_knns(knn_prefix, self.features,self.scores,cfg.knn_method, cfg.knn)
            print('knn graph build done: {}'.format(
                knn_prefix))
