...
from star.utils import sparse_mx_to_torch_sparse_tensor, build_knns,fast_knns2spmat,build_symmetric_adj,row_normalize,\
...
def train_gcn_reid(model, cfg, logger):
    # prepare dataset
    for k, v in cfg.model['kwargs'].items():
    ...
    if dataset.scores is not None:
        pre_scores=torch.FloatTensor(dataset.scores)
    else:
        pre_scores=None
    print('Have loaded the training data.')
    dataset_t = build_dataset(cfg.model['type'], cfg.test_data)
    # print('Have loaded the test data.')
    ...
    labels = torch.LongTensor(dataset.gt_labels)
    HEAD1 = HEAD(nhid=512,loss=cfg.loss)
    HEAD_test1 = HEAD_test(nhid=512)
    my_lr=cfg.optimizer['lr']
    ...
    OPTIMIZER = optim.SGD([{'params': model.parameters(),'weight_decay':1e-5},
                           {'params': HEAD1.parameters(),'weight_decay':1e-5}], lr=my_lr, momentum=0.9)
    print('the learning rate is ',my_lr)
    ...
    MODEL_ROOT = cfg.star_path+'/src/train_model'
    print('the model save path is', MODEL_ROOT)

    #prepare the test data
    target = "target"
    knn_path = cfg.prefix+"/knns/" + target + "/faiss_k_50.npz"
...
#shenme
        ...
        #for batch in range(20):
            #0.select ids
            # pre
            sample_batch=random.randint(0,cls_num-1)
            print('sample_batch:', sample_batch)
            sample_cluster_id = random.sample(list(cluster_id[sample_batch]), cfg.cluster_num)
            # change
            ...
            if cfg.knn_method =='knn_new':
                score=pre_scores[sample_id].numpy()
            else:
                score=None
            # print(sample_num)
            ...
            knn_prefix = os.path.join(cfg.star_path+"data/rebuild_knn")
